...
&nbsp;&nbsp;
<input type=submit name=susn value=SortByUsn>
</form>
END;
...
if (isset($_POST['ins']))
{
	try
	{
		...
		$usn=$_POST['t1'];
		$name=$_POST['t2'];
		$sql="INSERT into student values('$usn','$name')";
...
if (isset($_POST['sname']))
{
	...
	{  
		...
		echo "<table border><tr><th>USN<th>Name</tr>";
		for($j=0;$j<$cnt;$j++)
		{
			$usn=$student_data[$j][0];
			$name=$student_data[$j][1];
			echo "<tr><td>$usn<td>$name";
		}
	}
	catch(Exception $e)
	{
...
if (isset($_POST['susn']))
{
	try
	{
		...
		$student_data=selection_sort($student_data,0);
		echo "<table border><tr><th>USN<th>Name</tr>";
		for($j=0;$j<$cnt;$j++)
		{
			$usn=$student_data[$j][0];
